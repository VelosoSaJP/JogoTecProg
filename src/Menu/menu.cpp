#include "../../include/Menu/Menu.h"
    ...
    namespace Menu {
        ...
        Menu::Menu(const Estado::stateID ID, const sf::Vector2f tamBotao, const std::string nome, const unsigned int tamFonte):
            Ente(ID),/* listaBotaoTexto(), it()*/tamBotao(tamBotao), 
            tamJanela(), posFundo(sf::Vector2f(0.0f, 0.0f)),
            /*titulo(pGrafico->carregarFonte("Jungle++/fonte/menu.ttf"), nome, tamFonte)*/
             mouseSelecionado(false),vetorBotao()
        {
        ...
        void Menu::addBotao(const std::string info, const sf::Vector2f pos, const Estado::stateID ID, const sf::Color corSelecionado){
           /*
